...
2. Update the controller container on the nodes that are **not** the leader.
    ...
    > If you are using the systemd file given [here](setup/controller.md#create-systemd-file), you just need to restart the controller service using `systemctl restart drove.controller`

3. Set cluster to maintenance mode using `drove ... cluster maintenance-on`.
4. Update the leader controller.

    > If you are using the systemd file given [here](setup/controller.md#create-systemd-file), you just need to restart the leader controller service: `systemctl restart drove.controller`

5. Update the executors.

    > If you are using the systemd file given [here](setup/executor-setup.md#create-systemd-file), you just need to restart all executors: `systemctl restart drove.executor`
